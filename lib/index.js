...
(function () {
	 ...
	 */
	function Qbus (parent, proto) {
		var self = this;

		// Optional `new` keyword
		if (!self) {
			return new Qbus(parent, proto);
		}

		// Typechecking
		if ((parent && !parent.emit) || (proto && typeof proto !== 'object')) {
			throw new TypeError(
				'Usage: new Qbus([<`parent` = Object>, <`proto` = Object>])\n'+
				'Got:   new Qbus(<`' + typeof parent + '` = ' + parent + '>, <`' + typeof proto + '` = ' + proto + '>)'
			);
		}
		...
		if (proto) {
			proto.on = Qbus.prototype.on;
			proto.once = Qbus.prototype.once;
			proto.off = Qbus.prototype.off;
			proto.emit = Qbus.prototype.emit;
		}
