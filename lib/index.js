(function () {
	...
	function parse (expr) {
		...
		strSlashEnd = expr[expr.length - 1] === '/';
		...
		expr = expr.replace(/\/?[^\/]+?(?=\/|$)/g, function (match, index) {
			...
			// ? must be used with : to count (so we don't need to check for that)
			// * always counts
			if (match.indexOf('*') === -1 && (match.length <= 2 || !/(^|\/)+\:+?/.test(match))) {
				return match.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
			}

			var slashBeg = match[0] === '/' ? '/' : '',
				slashEnd = !!expr[match.length + index] || strSlashEnd,
				level = slashBeg ? match.substr(1) : match,
				firstChar = level[0],
			...
			// Handle wildcards
			if (level.length === 1 || firstChar === '*' || lastChar === '*') {
				// Match everything (beginning with possible "some" or ending with possible "thing") up until next /
				if (slashEnd) {
					parsed = level.replace(/\*/g, '([^/]+)?');
				}

				// Match everything (beginning with possible "some") up until end of string
				else if (level.length === 1 || lastChar === '*') {
					parsed = level.replace(/\*/g, '(.*?(?:(?=\/$)|(?=$)))?');
				} 

				// Match everything (ending with possible "thing") up until end of string
				else {
					parsed = level.replace(/\*/g, '(.*)?');
				}

				return slashBeg + parsed;
			}

			// Handle: [*som*thi*gawful]
			console.log('Handle something awful: Not yet implemented. Can\'t parse "teh string"');
		});
