...
(function () {
	 ...
	 * @return     {Object}  `this`
	 */
	Qbus.prototype.on = function (expr, async, handler) {
	 ...
	 * @return     {Object}          `this`
	 */
	Qbus.prototype.emit = function (defer, query, _args, func) {
		var paths = this.qbus.paths,
			i,
			needle,
			args,
		...
		// Optional `defer` variable
		if (defer !== true && defer !== false) {
			query = defer;
			defer = false;
		}

		// Handle deferred emit
		if (defer) {
			if (!func) {
				func = _args;
				args = [];
		...
		if (typeof query !== 'string' || (defer && (typeof func !== 'function' || !Array.isArray(args)))) {
			throw new TypeError(
				'\nusage: emit(<`defer` = False|Undefined>, <`query` = String>[, <`arg1` = *>], <`arg2` = *>, ...)\n'+
				'or:    emit(<`defer` = True>, <`query` = String>, <`args` = Array>, <`callback` = Function>)\n'
			);
		}
			...
			while (needle) {
				...
				if (parent) {
					for ( ; i < parent.length; ++i) {
						...
						// RegExp matching
						if (cur.expr.query) {
							if ((match = cur.expr.query(normal + slashEnd))) {
						...
						// String comparison
						else if (normal == cur.expr) {
							match = args;
						}

						if (match) {
							if (cur.async) {
								// Push 'resume' into the arguments to the handler to allow it to resume the emit when done
								match.push(function () {
									var ok = 1;
									return function () {
										ok && resume();
								...
								// Increment the index (so we don't land on this handler when resumed)
								i++;
							}

							// 
							if (!defer || func(cur, match, controls) !== false) {
								loop = !loop || !(exec(cur.handler, this, match) === false || cur.async);
							}

							// Discontinue if the handler returned false or if it's asynchronous
							if (loop === false) {
								return;
							}
						}
					...
					// We'll reset `i` down here so we can be sure that the for was completed.
					// Otherwise we'd restart at 0 after each async handler has resumed.
					i = 0;
				}
